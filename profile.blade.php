...
@section('css')
  <script type="text/javascript">
      var key_stripe = "{{ config('services.stripe.key') }}"; // Use a chave correta do Stripe aqui
  </script>
@endsection
...
@endif
              ...
              <div class="card-body px-lg-5 py-lg-5 position-relative">
                ...
                <form method="post" action="{{url('buy/subscription')}}" id="formSubscription">
                  ...
                  @foreach ($allPayment as $payment)
                    ...
                    <!-- BOTAO E FORMULARIO DE ADICIONAR/SALVAR CARTAO NA CONTA DIRETO PELO POPUP (REQUER SCRIPT ABAIXO) -->
  ...
  .custom-control {
    margin-bottom: 1rem; /* Ajusta o espaçamento entre os métodos de pagamento */
  }
</style>
...
@if ($payment->name == 'Stripe' && ! auth()->user()->pm_type != '')
  <div id="stripeContainer" class="@if ($allPayment->count() == 1 && $payment->name == 'Stripe')d-block @else d-none @endif">
    ...
    <button id="card-button" class="btn btn-1 btn-primary btn-block" data-secret="{{ $intent->client_secret }}">
      <span class="spinner-border spinner-border-sm" role="status" aria-hidden="true" style="display: none;" id="button-loader"></span>
      <span id="button-text">{{ trans('general.save_payment_card') }}</span>
    </button>
  </div>
@endif

@if ($payment->name == 'Paystack' && ! auth()->user()->paystack_authorization_code)
  <div id="paystackContainer" class="@if ($allPayment->count() == 1 && $payment->name == 'Paystack')d-block @else d-none @endif">
    <!-- Paystack Elements Placeholder -->
    <div id="paystack-element"></div>
    <div id="paystack-errors" class="alert alert-danger d-none" role="alert"></div>
    <div class="mt-2 text-center">
      <a href="{{ url('my/cards') }}"><i class="fa fa-long-arrow-alt-left"></i> {{ trans('general.go_back') }}</a>
    </div>
  </div>
@endif
...
@if ($settings->disable_wallet == 'on' && Helper::userWallet('balance') != 0 || $settings->disable_wallet == 'off')
  <div class="custom-control custom-radio mb-3">
    ...
    <label class="custom-control-label" for="radio0">
      <span><strong><i class="fas fa-wallet mr-1 icon-sm-radio"></i> {{ __('general.wallet') }}
        <span class="w-100 d-block font-weight-light">{{ __('general.available_balance') }}: <span class="font-weight-bold mr-1">{{Helper::userWallet()}}</span></span>
...
<div class="custom-control custom-radio mb-3">
  <input name="payment_gateway" required value="mercadopago" id="radio1" class="custom-control-input" type="radio">
  <label class="custom-control-label" for="radio1">
    <span><strong><i class="fas fa-money-check mr-1 icon-sm-radio"></i> {{ __('Pix / Outros') }}
      <span class="w-100 d-block font-weight-light">{{ __('Pague por Pix ou Outros Métodos') }}</span>
    </strong></span>
  </label>
</div>
...
<!-- Elemento onde o cartão será montado -->
<div id="card-element"></div>
...
<div id="messages"></div>
                    ...
                    <!-- FIM DE TESTE NOVAS FUNÇOES PAGAMENTO -->
                ...
                @endif
                          ...
                          <!-- FIM DO CONTEUDO DO MODAL POPUP -->





                        <!-- CODIGO DOS BOTOES PARA INSCRICAO -->
...
<div class="text-center">
    ...
    <a href="#" 
   type="button" 
   id="btn-assinar-mercado-pago" 
   class="btn btn-primary mt-4 w-100 subscriptionBtn" 
   data-amount="{{ $user->getPlan('monthly', 'price') }}"
...
@if ($plans->count())
        <!-- Link para expandir pacotes de assinatura -->
        ...
        <div class="collapse" id="collapseSubscriptionBundles">
            @foreach ($plans as $plan)
                <!-- Botão de assinatura para cada plano -->
                <button type="button" 
        class="btn btn-primary mt-2 w-100 subscriptionBtn" 
        data-amount="{{ Helper::formatPrice($plan->price, true) }}"
...
document.addEventListener('DOMContentLoaded', function () {
  const stripe = Stripe('{{ $stripeData['key'] }}');
  ...
  cardButton.addEventListener('click', async (e) => {
    ...
    if (error) {
      console.error(error);
      document.getElementById('messages').innerHTML = '<div class="alert alert-danger">Erro ao confirmar o cartão. Por favor, verifique os detalhes e tente novamente.</div>';
      buttonLoader.style.display = 'none';
      buttonText.style.display = 'inline-block';
      cardButton.disabled = false;
    } else {
      fetch('{{ route('add.update.payment.card') }}', {
        method: 'POST',
        headers: {
        ...
        if (data.success) {
          document.getElementById('messages').innerHTML = '<div class="alert alert-success fade-out">Cartão adicionado com sucesso!</div>';
          cardButton.classList.add('btn-success-with-check', 'btn-small-text');
          cardButton.textContent = 'Cartão Adicionado com Sucesso';
          cardButton.disabled = true;
        } else {
          document.getElementById('messages').innerHTML = '<div class="alert alert-danger">Falha ao adicionar cartão. Por favor, tente novamente.</div>';
          cardButton.disabled = false;
        }
        buttonLoader.style.display = 'none';
        buttonText.style.display = 'inline-block';
        ...
        document.getElementById('messages').innerHTML = '<div class="alert alert-danger">Erro ao processar a requisição. Por favor, tente novamente mais tarde.</div>';
        buttonLoader.style.display = 'none';
        buttonText.style.display = 'inline-block';
        cardButton.disabled = false;
      });
  ...
  paymentRadios.forEach(radio => {
    radio.addEventListener('change', () => {
      const selectedValue = document.querySelector('input[name="payment_gateway"]:checked').value;
      ...
      if (stripeContainer) {
        stripeContainer.classList.toggle('d-block', selectedValue === 'Stripe');
        stripeContainer.classList.toggle('d-none', selectedValue !== 'Stripe');
      }

      if (paystackContainer) {
        paystackContainer.classList.toggle('d-block', selectedValue === 'Paystack');
        paystackContainer.classList.toggle('d-none', selectedValue !== 'Paystack');
      }

      // Verifica se o método de pagamento selecionado é Pix
      if (selectedValue === 'mercadopago') {
        // Não faz nada aqui, pois o redirecionamento para o Mercado Pago é tratado separadamente
      }
    });
  });

  document.querySelectorAll('.subscriptionBtn').forEach(button => {
    button.addEventListener('click', function(event) {
      event.preventDefault();

      // Capturar o valor do atributo data-amount e data-interval
      const subscriptionAmount = this.getAttribute('data-amount');
      const planInterval = this.getAttribute('data-interval') || 'monthly'; // Usar 'monthly' como padrão se não estiver definido

      // Verifica se o método de pagamento selecionado é Cartão
      ...
      if (selectedPayment && selectedPayment.value === 'Stripe') {
        ...
        if (!cardElement._complete) {
          document.getElementById('messages').innerHTML = '<div class="alert alert-danger">É necessário adicionar um cartão de pagamento.</div>';
          return;
        }
      }

      // Realize uma solicitação ao backend para enviar o pagamento MercadoPago
      fetch('/send-mercadopago', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': '{{ csrf_token() }}' // Adicione o token CSRF se necessário
        },
        body: JSON.stringify({
          amount: subscriptionAmount, // Envia o valor da assinatura obtido do botão
          interval: planInterval, // Intervalo de assinatura obtido do atributo data-interval
          payment_gateway: 'Mercadopago' // Nome do gateway de pagamento
      ...
      .then(data => {
        if (data.success) {
          // Redirecionar para o URL de pagamento retornado pelo backend
          window.location.href = data.url;
        } else {
          // Tratar erros ou exibir mensagem de erro
          console.error('Erro ao iniciar pagamento com Mercado Pago:', data.errors);
          // Exibir uma mensagem de erro para o usuário, se necessário
          alert('Erro ao iniciar pagamento com Mercado Pago: ' + data.errors);
        }
      })
      .catch(error => {
        ...
        alert('Erro ao iniciar pagamento com Mercado Pago: ' + error.message);
                     ...
                     <!-- FIM DO CODIGO DOS BOTOES PARA INSCRICAO -->
