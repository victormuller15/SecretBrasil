  ...
  public function show()
    {
        ...
        // Encontrar o usuário para subscrição
        ...
        // Encontrar o gateway de pagamento Mercado Pago
        $payment = PaymentGateways::whereName('Mercadopago')->whereEnabled(1)->firstOrFail();

        // Configurar as credenciais do Mercado Pago
        \MercadoPago\SDK::setAccessToken($payment->key_secret);

        // Verificar se o plano já existe no Mercado Pago
        try {
            $planPreference = \MercadoPago\Plan::get_by_id($plan->name);
            $pricePlanOnMercadoPago = $planPreference->auto_recurring->transaction_amount;

            // Verificar se o preço do plano no Mercado Pago difere do preço local
            if ($pricePlanOnMercadoPago != $plan->price) {
                // Atualizar o plano com o novo preço
                $planPreference->auto_recurring->transaction_amount = $plan->price;
                $planPreference->update();
            }
        } catch (\MercadoPago\Exceptions\MpException $e) {
            // Criar um novo plano no Mercado Pago se ele não existir
            $this->createPlan($plan, $payment);
        }

        // Criar ou atualizar a assinatura do usuário
        try {
            // Verificar se há pagamento incompleto
            if (auth()->user()
                ->userSubscriptions()
                ->where('mercadopago_price', $plan->name)
                ->whereMeracdoPagoStatus('incomplete')
                ->first()
            ) {
                return response()->json([
                    'success' => false,
            ...
            $subscription->mercadopago_price = $plan->name;
            $subscription->save();
    ...
    /**
     * Criar um plano no Mercado Pago
     *
     * @param $plan
     * @param $payment
     */
    private function createPlan($plan, $payment)
    {
        \MercadoPago\SDK::setAccessToken($payment->key_secret);
        ...
        $item->currency_id = config('settings.currency_code');
  ...
  public function profile($slug, $media = null)
{
    ...
    // Prepare Stripe setup intent and key for view
    $stripeData = [
    ...
    return view('users.profile', [
        ...
        'intent' => $stripeData['intent'] // Adicionei esta linha para garantir que a variável $intent seja passada para a view
    ]);
}
  ...
  public function sendMercadopago(Request $request)
{
    try {
        // Get Payment Gateway
        ...
        $taxes = $this->settings->tax_on_wallet ? ($request->amount * auth()->user()->isTaxable()->sum('percentage') / 100) : 0;

        $amountFixed = number_format($request->amount + ($request->amount * $fee / 100) + $cents + $taxes, 2, '.', '');
        ...
        // External reference
        $preference->external_reference = 'userId=' . auth()->user()->id . ',amountOriginal=' . $request->amount . ',userTaxes=' . auth()->user()->taxesPayable() . '';
